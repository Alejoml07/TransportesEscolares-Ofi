...
def guardarUsuario(request):
    ...
    try:
        ...
        messages.warning(request, "No se pudo realizar el registro...")
        return render(request, 'transportes/login/usuarios/registrarUsuario.html')
...
def eliminarUsuario(request, id):
    ...
    try:
        p =  Cliente.objects.get(pk = id)
        if p.rol == 'C' or p.rol == 'P':
            p.delete()
            messages.success(request, "Usuario eliminado correctamente!!")
        else:
            messages.warning(request, "No puede eliminar un administrador")
...
def listarBeneficiarioCli(request):
    """Se usa para obtener los datos de todos los beneficiario y enviarlos a un template 
    ...
    if login and (login[2] == "A" or login[2] == "C"):
        ...
        return render(request, 'transportes/login/beneficiarios/listarBeneficiarioCli.html', contextob)
    else:
        if login and (login[2] != "A" and login[2] != "C"):
            messages.warning(request, "Usted no tiene autorización para acceder al módulo...")
            return redirect('transportes:index')
...
def guardarBeneficiario(request):
    ...
    if login[2] == 'C':
        return redirect('transportes:listarBeneficiarioCli')
    else:
...
def registrarComentarios(request,id):
...
def registrarPeticiones(request, id):
...
def eliminar(request,id):
    ...
    return redirect('transportes:listarPeticionesAceptadas')
